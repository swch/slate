...
```json
{
  "card_link_url": "https://api.example.com/card_links/ondemand/abc123",
  "cardholder_long_token_expiration": "2026-07-16T00:00:00.000Z",
  "card_link_id": 1234567890
}
```
...
In response to a properly formed POST request to the OnDemand endpoint containing this information, the user will receive a card link containing a fully formed URL that can be accessed immediately, along with the card link ID and the expiration date timestamp.

### Body parameters

Parameter | Subproperty | Type | Required | Default | Description
-------- | ---- | ---- | --------- | ------- | ----------
...
Parameter | Type | Description
-------- | ---- | ----------
card_link_url | string | Fully formed URL for the card link.
cardholder_long_token_expiration | string | Expiration date of the cardholder's long token.
card_link_id | number | ID of the created card link.

<aside class="notice">Calls to /card_links/ondemand require the cardholder's personal cardholder_safe_key header to encrypt and save card and account data.</aside>
